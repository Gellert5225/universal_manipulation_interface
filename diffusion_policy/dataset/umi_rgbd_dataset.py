...
Extends UmiDataset to include depth images alongside RGB.
"""
...
class UmiRGBDDataset(BaseDataset):
    """
    UMI dataset with RGB and depth images.
    
    Extends the standard UMI dataset to load and normalize depth images.
    """
    ...
    def __init__(self,
        ...
        max_depth: float=10.0,  # Max depth value in meters for normalization
    ):
        ...
        # Combine RGB and depth keys for the sampler
        all_image_keys = rgb_keys + depth_keys

        sampler = SequenceSampler(
            shape_meta=shape_meta,
            replay_buffer=replay_buffer,
            rgb_keys=all_image_keys,  # Sampler treats depth like RGB for loading
    ...
    def get_validation_dataset(self):
        ...
        all_image_keys = self.rgb_keys + self.depth_keys
        val_set.sampler = SequenceSampler(
            shape_meta=self.shape_meta,
            replay_buffer=self.replay_buffer,
            rgb_keys=all_image_keys,
            lowdim_keys=self.sampler_lowdim_keys,
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        # Depth images - identity normalizer since we normalize during loading
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key in self.depth_keys:
            if not key in data:
                continue
            depth_data = data[key].astype(np.float32)
            
            # Handle invalid depth values
            depth_data = np.nan_to_num(depth_data, nan=self.max_depth, posinf=self.max_depth, neginf=0.0)
            
            # Normalize to [0, 1]
            depth_data = np.clip(depth_data / self.max_depth, 0.0, 1.0)
            
            # Ensure proper shape: T,H,W -> T,1,H,W or T,H,W,1 -> T,1,H,W
            if len(depth_data.shape) == 3:
                depth_data = depth_data[:, np.newaxis, :, :]
            elif len(depth_data.shape) == 4 and depth_data.shape[-1] == 1:
                depth_data = np.moveaxis(depth_data, -1, 1)
            
            obs_dict[key] = depth_data
            del data[key]
