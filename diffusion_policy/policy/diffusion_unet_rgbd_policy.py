...
from diffusion_policy.model.vision.rgbd_obs_encoder import RGBDObsEncoder
from diffusion_policy.common.pytorch_util import dict_apply


class DiffusionUnetRGBDPolicy(BaseImagePolicy):
    """
    Diffusion Policy with RGBD observations.
    
    Uses RGBDObsEncoder to process both RGB and depth images,
    then generates actions via diffusion.
    """
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], fixed_action_prefix: torch.Tensor=None) -> Dict[str, torch.Tensor]:
        ...
        cond_data = torch.zeros(size=(B, self.action_horizon, self.action_dim), device=device, dtype=dtype)
    ...
    def compute_loss(self, batch):
        ...
        if self.train_diffusion_n_samples != 1:
            ...
            # each sample will later have a different noise sample, effectively training 
